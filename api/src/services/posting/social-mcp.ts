...
export interface PublishPayload {
  ...
  // Explicit subject/html for campaign sends — bypasses content-line parsing
  emailSubject?: string;
...
async function publishToEmail(p: PublishPayload): Promise<PublishResult> {
  ...
  // Resolve subject and body — explicit fields take priority over content-line parsing
  let subject: string;
  ...
  if (p.emailSubject) {
    ...
    subject = (newlineIdx > -1 ? p.content.slice(0, newlineIdx) : p.content.slice(0, 100))
      .replace(/^Subject:\s*/i, "")
      .trim();
    plainBody = newlineIdx > -1 ? p.content.slice(newlineIdx + 1).trim() : p.content;
  }
  ...
  const fromName = addrMatch ? addrMatch[1]!.replace(/^"|"$/g, "").trim() : undefined;

  // EmailService now accepts credentials directly in the constructor — no config
  // patching or mutex needed. Each call is fully isolated.
  const svcUrl = pathToFileURL(
    path.join(smcpDistRoot(), "services/email-service.js"),
  ).href;
  const { EmailService } = (await import(svcUrl)) as {
    EmailService: new (creds: {
  ...
  const service = new EmailService({
    mailer: p.emailMailer,
    fromAddress,
    fromName: fromName || undefined,
    smtpHost: p.emailSmtpHost,
    smtpPort: p.emailSmtpPort,
    smtpUsername: p.emailSmtpUsername,
    smtpPassword: p.emailMailer === "smtp" ? p.accessToken : undefined,
    smtpEncryption: p.emailSmtpEncryption as "tls" | "ssl" | "none" | undefined,
    sendgridApiKey: p.emailMailer === "sendgrid" ? p.accessToken : undefined,
    mailgunApiKey: p.emailMailer === "mailgun" ? p.accessToken : undefined,
    mailgunDomain: p.emailMailgunDomain,
  });

  // Send to each recipient separately — prevents address leakage across recipients
  await Promise.all(
    p.emailTo.map((to) =>
      service.send(to, subject, plainBody, p.emailHtmlBody),
    ),
  );
